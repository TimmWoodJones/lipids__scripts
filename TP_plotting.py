...
for i in range(N):
    ...
    if mode == 'heat' or mode == 'all':
        ...
        if dir_check(directory)==True:
            temp,press,cdata=calc(directory)
            #color=colors[i%int(len(colors))]
            marker=markers[i%int(len(markers))]
            plt.scatter(temp,press,c=cdata,marker=marker,cmap=cmap,s=size,vmin=0,vmax=1)
            k+=1
        ...
        for d in obj:
            if d.is_dir()==True:
                ...
                if dir_check(directory)==True:
                    ...
                    #color=colors[i%int(len(colors))]
                    marker=markers[i%int(len(markers))]
                    plt.scatter(press,temp,c=cdata,marker=marker,cmap=cmap,s=size,vmin=0,vmax=1)
                    k+=1
...
fig1.savefig('TP_plot_'+mode+'_'+cmode+'_color.png')
